...
function buildTTSConfig(ttsVendor, speedOverride) {
  ...
  if (ttsVendor === 'microsoft') {
    ...
    return {
      params: {
        ...
        voice_setting: {
          voice_id: process.env.MINIMAX_VOICE_ID || 'English_captivating_female1',
          speed: ttsSpeed,
        },
      },
    };
 ...
 */
function getPresetString(ttsVendor) {
  const asrLlm = process.env.AGORA_PRESET_ASR_LLM || 'deepgram_nova_3,openai_gpt_4o_mini';

  if (ttsVendor === 'preset_minimax') {
    return `${asrLlm},minimax_speech_2_6_turbo`;
  } else if (isPresetTTS(ttsVendor)) {
    return process.env.AGORA_PRESET || `${asrLlm},openai_tts_1`;
  }
...
function buildAvatarConfig(avatarVendor) {
  ...
  if (avatarVendor === 'anam') {
    ...
    return {
      ...
      params: {
        ...
        avatar_id: process.env.LEMONSLICE_AVATAR_ID,
        agora_uid: process.env.AVATAR_AGORA_UID || '102',
        agora_token: '__PLACEHOLDER__',
        sample_rate: 24000,
        quality: 'high',
...
export async function joinAgent(channelName, agentName, ttsVendor, avatarVendor = 'none', extraConfig = {}) {
  ...
  const ttsConfig = buildTTSConfig(vendor, extraConfig.ttsSpeed);
  ...
  const body = {
    ...
    properties: {
      ...
      tts: ttsConfig,
      ...(hasAvatar && (() => {
        const avatarConfig = buildAvatarConfig(avatarVendor);
        if (avatarConfig) {
  ...
  console.log(`[Agora] JOIN ${channelName} (tts=${vendor}, avatar=${avatarVendor})`);
 ...
 */
export async function interrupt(agentId) {
  const appId = getAppId();
  const authToken = getAuthToken();
