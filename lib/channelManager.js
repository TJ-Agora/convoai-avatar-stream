...
export async function deleteChannel(id) {
  ...
  if (meta?.agentId) {
    try {
      await agora.leaveAgent(meta.agentId);
    } catch (err) {
      console.error(`[ChannelManager:${id}] leaveAgent during delete failed:`, err.message);
    }
  }
  const p = redis.pipeline();
  for (const s of ['meta', 'messages', 'msgseq', 'queue', 'batch', 'participants', 'welcomed', 'welcomeq', 'turns', 'speaktexts']) {
    p.del(k(id, s));
  }
...
export async function getChannel(id) {
  ...
  async function speakNow(text, type) {
    ...
    try {
      ...
      await agora.speak(m.agentId, text, 'APPEND', true);
    } catch (err) {
  ...
  async function markSpeechDone() {
    ...
    // Land the just-finished answer FIRST: syncAgentHistory inserts it at ITS
    // anchor. Dispatching the next question first would overwrite the anchor
    // and the answer would mis-place (observed in prod as "my comment showed
  ...
  async function drainQueue() {
    ...
    try {
      ...
      try {
        await agora.think(m.agentId, q.text);
      } catch (err) {
  ...
  async function closeBatchWindow() {
    ...
    try {
      ...
      try {
        await agora.think(m.agentId, buildBatchPrompt(batch));
      } catch (err) {
  ...
  async function syncAgentHistory() {
    ...
    try {
      const history = await agora.getAgentHistory(m.agentId);
  ...
  async function stop() {
    ...
    if (m.agentId) {
      try {
        await agora.leaveAgent(m.agentId);
